...
- 最简可用筛选应收敛为：
  ...
  - 关注状态
  - 排序
- Market Explorer 的预设模式保留两类即可：
  - `全部盘口`
  - `关注队列`
- `异常彩票` 不再作为 Market Explorer 里的预设模式，而是收口到 `Rules Settings` 里的系统规则。
- Market Explorer 只保留命中后的市场上下文，不再承担异常彩票规则入口。

### 3. 页面呈现

- 主承载页改为 `Rules Settings`，并与 `盘口斩杀`、`带量定价` 处于同一层级。
- 页面应同时让人一眼看出三件事：
  - 这条规则怎么触发
  - 这条规则监控哪些市场范围
  - 当前有哪些样例市场正在命中或接近命中
- 因此当前实现调整为：
  - 系统规则内建项：`abnormal_lottery`
  - 规则说明与触发引导：超低价区间、确认终点、观察窗口、确认来源
  - 规则预览样例：用市场上下文展示参考价、当前价、确认路径和有效数量
  - Market Explorer 只保留辅助上下文：推高幅度、确认来源、价格路径，以及右侧检视面板
